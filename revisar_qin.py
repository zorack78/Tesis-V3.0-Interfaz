#!/usr/bin/env python3
"""
Script para revisar estructura del archivo de caudal de entrada.
"""
...
def revisar_qin():
    file_path = Path("data/raw/BD_Qin_m3_UTC.csv")
    ...
    if total_nulls > 0:
        ...
        if len(null_cols) <= 10:
            ...
            print(f"   (Mostrando primeras 5 columnas con más nulos)")
            for col, count in null_cols.nlargest(5).items():
                print(f"   {col}: {count} ({100*count/len(df):.1f}%)")
    else:
