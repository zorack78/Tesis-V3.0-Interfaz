...
class AdvancedWaterDemandModel:
    ...
    def select_best_features(self, df_features, target_col):
        ...
        seasonal_features = [
            'is_summer', 'is_winter', 'is_peak_hour', 'is_low_hour'
        ]
        
        # Features de calendario
        calendar_features = []
        calendar_cols = [
            'feriado', 'es_fin_de_semana', 'temporada_turistica_alta',
            'festival_vina', 'elecciones', 'vacaciones_escolares'
        ]
        for feature in calendar_cols:
        ...
        all_features = (
            basic_features + lag_features + rolling_features + 
...
def main():
    ...
    print(f"✅ Datos cargados: {len(train_df)} train, "
    ...
    create_advanced_prediction_plot(
        test_features, y_test, predictions_result, test_metrics
    )
...
def create_advanced_prediction_plot(test_features, y_test, 
                                   predictions_result, metrics):
    ...
    # Banda de confianza
    plt.fill_between(
        range(len(y_test_sample)), pred_lower_sample, pred_upper_sample,
    ...
    plt.subplot(2, 3, 3)
    scatter = plt.scatter(
        y_test, y_pred, c=predictions_result['confidence_width'], 
    ...
    plt.subplot(2, 3, 5)
    plt.scatter(predictions_result['confidence_width'], errors, 
                alpha=0.6, s=25)
