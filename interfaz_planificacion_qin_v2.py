...
"""
    ...
    def _generar_grafico_testing_tres_modelos(self, resultados, predicciones,
                                               y_test, timestamps):
        """Genera gráfico comparativo para los 3 modelos ML - V2.0 con MAPE y Scatter Plots"""
        ...
        # Crear subplots: 3 filas x 4 columnas
        # Fila 1: 4 barras de métricas (todo el período de testing)
        # Fila 2: Serie temporal ocupando todo el ancho (últimos 7 días)
        # Fila 3: 3 Scatter plots más grandes y centrados
        fig = make_subplots(
            rows=3, cols=4,
            subplot_titles=(
                'R² Score', 'RMSE (m³/h)', 'MAE (m³/h)', 'MAPE (%)',
                'Serie Temporal - Últimos 7 Días del Período de Testing', '', '', '',
                'XGBoost V3.0 - Predicción vs Real', 'RandomForest - Predicción vs Real', 'LightGBM - Predicción vs Real', ''
            ),
        ...
        # MAPE en col 4
        fig.add_trace(go.Bar(
            x=modelos, y=mape_vals, marker_color=colors,
            showlegend=False, text=[f'{v:.2f}%' for v in mape_vals],
            textposition='outside',
            textfont=dict(size=12),
            cliponaxis=False
        ), row=1, col=4)
        
        # Fila 2: Serie temporal (todo el ancho, 4 columnas)
        ...
        nombres_display = {
            'XGBoost V3.0': 'Predicción XGBoost',
            'RandomForest': 'Predicción RandomForest',
            'LightGBM': 'Predicción LightGBM'
        }
        for i, (nombre, color) in enumerate(zip(modelos, colors)):
            if nombre in predicciones:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    y=y_pred_plot,
                    mode='lines',
                    name=nombres_display.get(nombre, nombre),
                    line=dict(color=color, width=2, dash='dot'),
                    hovertemplate=f'%{{x|%d/%m %H:%M}}<br>{nombre}: %{{y:,.0f}} m³/h<extra></extra>'
                ), row=2, col=1)
        
        # Fila 3: Scatter plots con métricas
        min_val = min(y_test.min(), min([predicciones[m].min() for m in modelos if m in predicciones]))
        max_val = max(y_test.max(), max([predicciones[m].max() for m in modelos if m in predicciones]))
        
        # Scatter 1: XGBoost
        nombre_xgb = 'XGBoost V3.0'
        if nombre_xgb in predicciones:
            y_pred_xgb = predicciones[nombre_xgb]
            fig.add_trace(go.Scatter(
            ...
            # Añadir métricas como anotación
            idx = modelos.index(nombre_xgb)
            metrics_text = (
                f"<b>Métricas (Todo el Período)</b><br>"
                f"R²: {r2_vals[idx]:.4f}<br>"
                f"RMSE: {rmse_vals[idx]:,.0f} m³/h<br>"
                f"MAE: {mae_vals[idx]:,.0f} m³/h<br>"
                f"MAPE: {mape_vals[idx]:.2f}%"
            )
            fig.add_annotation(
                x=0.05, y=0.97, xref='x6', yref='y6 domain',
                text=metrics_text,
                showarrow=False,
        ...
        # Scatter 2: RandomForest
        nombre_rf = 'RandomForest'
        if nombre_rf in predicciones:
            y_pred_rf = predicciones[nombre_rf]
            fig.add_trace(go.Scatter(
                x=y_test, y=y_pred_rf,
                mode='markers',
                marker=dict(color=colors[1], size=4, opacity=0.6),
                name=nombre_rf,
                showlegend=False,
                hovertemplate=(
                    'Real: %{x:,.0f} m³/h<br>'
            ...
            fig.add_annotation(
                x=0.05, y=0.97, xref='x7', yref='y7 domain',
                text=metrics_text,
                showarrow=False,
                font=dict(size=10, color=colors[1]),
                bgcolor='rgba(255,255,255,0.9)',
        ...
        # Scatter 3: LightGBM
        nombre_lgb = 'LightGBM'
        if nombre_lgb in predicciones:
            y_pred_lgb = predicciones[nombre_lgb]
            fig.add_trace(go.Scatter(
                x=y_test, y=y_pred_lgb,
                mode='markers',
                marker=dict(color=colors[2], size=4, opacity=0.6),
                name=nombre_lgb,
                showlegend=False,
                hovertemplate=(
                    'Real: %{x:,.0f} m³/h<br>'
            ...
            fig.add_annotation(
                x=0.05, y=0.97, xref='x8', yref='y8 domain',
                text=metrics_text,
                showarrow=False,
        ...
        # Actualizar layouts
        # Fila 1: Métricas (con rango extendido para valores sobre barras)
        fig.update_yaxes(
            title_text="R²", row=1, col=1,
            range=[0, max(r2_vals) * 1.15]
        )
        fig.update_yaxes(
            title_text="RMSE (m³/h)", row=1, col=2,
            range=[0, max(rmse_vals) * 1.15]
        )
        fig.update_yaxes(
            title_text="MAE (m³/h)", row=1, col=3,
            range=[0, max(mae_vals) * 1.15]
        )
        fig.update_yaxes(
            title_text="MAPE (%)", row=1, col=4,
            range=[0, max(mape_vals) * 1.15]
        )
        
        # Fila 2: Serie temporal (col 1 con colspan 4)
        fig.update_xaxes(
            title_text="Fecha/Hora",
            row=2, col=1,
            title_font=dict(size=12)
        )
        fig.update_yaxes(
            title_text="Demanda Qout (m³/h)",
            row=2, col=1,
            title_font=dict(size=12)
        )
        
        # Fila 3: Scatter plots (cols 1, 2, 3)
        for col in [1, 2, 3]:
            fig.update_xaxes(
                title_text="Demanda Real (m³/h)",
                row=3, col=col,
                title_font=dict(size=11)
            )
            fig.update_yaxes(
                title_text="Predicción (m³/h)",
                row=3, col=col,
                title_font=dict(size=11)
            )
        ...
        fig.update_layout(
            height=1150,
            title_text=(
                "<b>Evaluación Comparativa de 3 Modelos ML - V2.0</b><br>"
                "<sub>Período de Testing Completo: 26/06/2025 - 30/09/2025 (2,255 horas)</sub>"
            ),
            hovermode='closest',
            showlegend=True,
            legend=dict(
